...
if getgenv().Settings and getgenv().Settings.JoinTeam then
    ...
    for i, v in pairs(game:GetService("Workspace")["_WorldOrigin"].Locations:GetChildren()) do
        pcall(function()
            ...
            if IslandESP and v.Name ~= "Sea" then
                ...
                name.Text =
                    v.Name.." < "..round(
                    (game.Players.LocalPlayer.Character.Head.Position - v.Position).Magnitude / 3
                    ).." >"
            ...
            pcall(function()
                ...
                if not isnil(v.Character) then
                    ...
                    if ESPPlayer then
                        ...
                        if not isnil(v.Character.Head)
                        and not v.Character.Head:FindFirstChild("NameEsp" .. Number) then
                            ...
                            local Text = v.Character.Head:FindFirstChild("NameEsp" .. Number)
                            ...
                            if Text then
                                ...
                                if v.Team == game.Players.LocalPlayer.Team then
                                    Text.TextLabel.TextColor3 = Color3.fromRGB(0, 0, 255)
                                else
                                    Text.TextLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
                                end
...
function topos(aL)
    pcall(function()
        ...
        if player and character and humanoid and rootPart and humanoid.Health > 0 then
            ...
            if getgenv().ResetBypassTeleport and Distance >= 5000 then
                local nodes = buildNodes()
                if #nodes > 0 then
                    local targetIslandName = nearestIsland(targetPos, nodes)
                    if targetIslandName then
                        DC_FastHopTo(targetIslandName)
            ...
            if IgnoreY and (b1.Position - targetCFrameWithDefaultY.Position).Magnitude > 5 then
                ...
                local tweenInfo = TweenInfo.new(
                    (targetPos - rootPart.Position).Magnitude / TweenSpeed,
                    Enum.EasingStyle.Linear
                )

                getgenv().currentTween = tweenService:Create(
                    rootPart,
                    tweenInfo,
...
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if _G.StartFarm and _G.ChooseWP then
...
local Window = Library:MakeWindow({
  Title = "uoɯǝɐɹoD Hub : Blox Fruits",
  SubTitle = "By Phuocdepzai",
  SaveFolder = "DoraemonHub.config"
})
...
ImageButton.Image = "rbxassetid://76924610417991"
...
Setting:AddToggle({
    Name = "Reset Bypass Teleport [Beta]",
    Description = "This is a test, so it may contain errors",
    Default = false,
    Callback = function(Value)
        getgenv().ResetBypassTeleport = Value
    end
})

getgenv().ResetBypassTeleport = false
Setting:AddToggle({
...
BringEnemy = function()
    ...
    local targetMobName = getgenv().CurrentMobName or getgenv().CurrentBoneName
    
    if not targetMobName and getgenv().CurrentBoneTarget and getgenv().CurrentBoneTarget.Parent then
        targetMobName = getgenv().CurrentBoneTarget.Name
    end
    ...
    for _, v in pairs(enemies:GetChildren()) do
        ...
        if hum and rootPart and hum.Health > 0 and PosMon then
            local isTargetMatch = not targetMobName or (v.Name == targetMobName)
            
            if isTargetMatch and (rootPart.Position - PosMon).Magnitude <= 400 then
                rootPart.CFrame = CFrame.new(PosMon)
...
local SubNPC = CFrame.new(-16267.8, 25.4, 1373.2)
...
local function IsInSubmergedIsland()
    ...
    local spawn = player.Data:FindFirstChild("SpawnPoint")
        or player.Data:FindFirstChild("CurrentIsland")

    if not spawn then
        return false
    end

    local name = tostring(spawn.Value):lower()

    return string.find(name,"submerged")
    or string.find(name,"underwater")
end
...
local function GoSubmerged()
    ...
    if IsInSubmergedIsland() then return end

    repeat
        task.wait()
        topos(SubNPC)
    until not getgenv().AutoFarmLevel
    or (hrp.Position - SubNPC.Position).Magnitude <= 80
    ...
    while task.wait(0.2) do
        if getgenv().AutoFarmLevel then
            pcall(function()
                ...
                if not Mon or not NameMon or not CFrameQuest then
                    return
                end
...
Farm:AddToggle({
    ...
    Default = false,
    Callback = function(Value)
        getgenv().AutoFarmNearestMobs = Value
    end
})

spawn(function()
    while task.wait() do
        if getgenv().AutoFarmNearestMobs then
            pcall(function()
...
spawn(function()
    while task.wait(Sec) do
        ...
        if getgenv().AutoPirateRaid
        and Root then

            pcall(function()

                local CFrameCastleRaid = CFrame.new(
                    -5496.17432,
                    313.768921,
                    -2841.53027,
                    0.924894512,
                ...
                if (CastlePos.Position - Root.Position).Magnitude <= 500 then
                    ...
                    for _,v in pairs(workspace.Enemies:GetChildren()) do
                        ...
                        and (v.HumanoidRootPart.Position - Root.Position).Magnitude <= 2000 then
                            ...
                            repeat
                                ...
                                Attack.Kill(
                                    v,
                                    getgenv().AutoPirateRaid
                                )

                            until not getgenv().AutoPirateRaid
                                or not v.Parent
...
local Sec = Sec or 0.1  Farm:AddToggle({     Name = "Auto Farm Bone",     Default = false,     Callback = function(Value)         getgenv().AutoFarmBone = Value         if not Value then             getgenv().CurrentBoneTarget = nil             getgenv().CurrentBoneName = nil         end     end })  getgenv().CurrentBoneTarget = nil getgenv().CurrentBoneName = nil  local function TweenBone(cf)     local player = game.Players.LocalPlayer     local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")      if hrp and (hrp.Position - cf.Position).Magnitude > 10 then           if type(topos) == "function" then             topos(cf)           end     end end  spawn(function()     while true do         task.wait(Sec)         if getgenv().AutoFarmBone then             pcall(function()                 local player = game.Players.LocalPlayer                   local character = player.Character or player.CharacterAdded:Wait()                   local root = character:WaitForChild("HumanoidRootPart", 5)                                  if not root then return end                  local questUI = player.PlayerGui:WaitForChild("Main"):WaitForChild("Quest")                    local BonesTable = {                       "Reborn Skeleton",                       "Living Zombie",                       "Demonic Soul",                       "Posessed Mummy"                   }                    local enemies = workspace:FindFirstChild("Enemies")                   local bone = nil                    if getgenv().CurrentBoneTarget and getgenv().CurrentBoneTarget.Parent then                     local hum = getgenv().CurrentBoneTarget:FindFirstChild("Humanoid")                     if hum and hum.Health > 0 then                         bone = getgenv().CurrentBoneTarget                     else                         getgenv().CurrentBoneTarget = nil                         getgenv().CurrentBoneName = nil                     end                 end                  if not bone and enemies then                       local closestDist = math.huge                     for _, mob in pairs(enemies:GetChildren()) do                         local hum = mob:FindFirstChild("Humanoid")                         local mobRoot = mob:FindFirstChild("HumanoidRootPart")                                                  local isValidName = false                         for _, name in ipairs(BonesTable) do                             if mob.Name == name then                                 isValidName = true                                 break                             end                         end                          if isValidName and hum and mobRoot and hum.Health > 0 then                             local dist = (mobRoot.Position - root.Position).Magnitude                             if dist < closestDist then                                 closestDist = dist                                 bone = mob                                 getgenv().CurrentBoneTarget = mob                                 getgenv().CurrentBoneName = mob.Name                             end                         end                     end                   end                    if bone then                       if getgenv().AcceptQuest and not questUI.Visible then                           local questPos = CFrame.new(-9516.99316, 172.017181, 6078.46533)                           TweenBone(questPos)                            local timeout = 0                           repeat                               task.wait(0.2)                               local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")                               if hrp and (questPos.Position - hrp.Position).Magnitude <= 15 then                                   break                               end                               timeout += 1                           until not getgenv().AutoFarmBone or timeout >= 25                            local questData = {                               {"StartQuest","HauntedQuest2",2},                               {"StartQuest","HauntedQuest2",1},                               {"StartQuest","HauntedQuest1",1},                               {"StartQuest","HauntedQuest1",2}                           }                            pcall(function()                               game.ReplicatedStorage.Remotes.CommF_:InvokeServer(unpack(questData[math.random(1, 4)]))                           end)                       end                        repeat                           task.wait()                            if not bone.Parent or not bone:FindFirstChild("Humanoid") or bone.Humanoid.Health <= 0 then                               getgenv().CurrentBoneTarget = nil                             getgenv().CurrentBoneName = nil                             break                           end                            if bone:FindFirstChild("HumanoidRootPart") then                               TweenBone(bone.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))                           end                            if Attack and type(Attack.Kill) == "function" then                             Attack.Kill(bone, getgenv().AutoFarmBone)                           end                      until not getgenv().AutoFarmBone or (getgenv().AcceptQuest and not questUI.Visible)                    else                       TweenBone(CFrame.new(-9495.6806640625, 453.58624267578125, 5977.3486328125))                   end               end)           end       end end) 
            ...
            pcall(function()
                ...
                and Boss.Humanoid.Health > 0 then
                    ...
                    repeat
                        ...
                        Attack.Kill(
                            Boss,
                            getgenv().AutoSoulReaper
                        )

                    until not getgenv().AutoSoulReaper
                        or not Boss.Parent
                    ...
                    if not GetBP("Hallow Essence") then
                        ...
                        repeat
                            ...
                            if hrp
                            and (hrp.Position - summonPos.Position).Magnitude <= 10 then
                                break
                            end
                        ...
                        pcall(function()
                            local Tool = plr.Character
                                and plr.Character:FindFirstChild("Hallow Essence")
        ...
        if _G.AutoFarmChest then
            pcall(function()
                ...
                for i = 1, #Chests do
                    ...
                    local Magnitude = (
                        Chest:GetPivot().Position - Position
                    ).Magnitude

                    if not SelectedIsland
                    or Chest:IsDescendantOf(SelectedIsland) then

                        if not Chest:GetAttribute("IsDisabled")
...
local function TweenKatakuri(cf)
    local plr = game.Players.LocalPlayer
    local char = plr.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")

    if hrp and (hrp.Position - cf.Position).Magnitude > 50 then
        topos(cf)
    end
end
    ...
    while task.wait() do
        if _G.Auto_Cake_Prince then
            pcall(function()
                ...
                local questUI = player.PlayerGui:WaitForChild("Main"):WaitForChild("Quest")
                local enemies = workspace:FindFirstChild("Enemies")
                ...
                if not bigMirror then
                    TweenKatakuri(CFrame.new(-2077,252,-12373))
                    return
                end
                ...
                if otherPart.Transparency == 0
                or (enemies and enemies:FindFirstChild("Cake Prince")) then
                    ...
                    if v then
                        ...
                        if getgenv().AcceptQuest
                        and not questUI.Visible then
                            ...
                            repeat
                                ...
                                timeout = timeout + 1
                            until not root
                                or (questPos.Position - root.Position).Magnitude <= 15
                                or timeout >= 50
                        ...
                        if enemies and root then
                            for _, mob in pairs(enemies:GetChildren()) do
                                ...
                                and mobRoot
                                and hum.Health > 0
                                and (mobRoot.Position - root.Position).Magnitude <= 50 then
                                    nearMob = true
                                    break
                                end
                            end
...
Farm:AddSection("Farm Tyrant")

getgenv().AutoFarmTyrant = false

Farm:AddToggle({
    Name = "Farm Tyrant of the Skies",
    Default = false,
    Callback = function(Value)
        getgenv().AutoFarmTyrant = Value
    end
})

local function TweenTyrant(cf)
    local hrp = game.Players.LocalPlayer.Character
        and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
...
spawn(function()
    while task.wait() do
        if getgenv().AutoFarmTyrant
        and getgenv().AutoFarm
        and World3 then
            ...
            pcall(function()
                ...
                and Boss.Humanoid.Health > 0 then
                    ...
                    until not getgenv().AutoFarmTyrant
                    ...
                    local TyrantSpawn = ReplicatedStorage:FindFirstChild("Tyrant of the Skies")

                    if TyrantSpawn
                    and TyrantSpawn:FindFirstChild("HumanoidRootPart") then

                        TweenTyrant(
                            TyrantSpawn.HumanoidRootPart.CFrame
                            * CFrame.new(0,30,0)
                        )
...
Farm:AddToggle({
    ...
    Default = false,
    Callback = function(Value)
        getgenv().AutoFarmAura = Value
    end
})

local function TweenAura(cf)
    local hrp = game.Players.LocalPlayer.Character
        and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
...
spawn(function()
    while task.wait() do
        ...
        if getgenv().AutoFarmAura
        and getgenv().AutoFarm then
...
Farm:AddSection("Event Dog House")
Farm:AddToggle({
    Name = "Auto Farm Mob Event Dog House",
    Description = "Turn it on when entering the dog house",
    Default = false,
    Callback = function(Value)
...
spawn(function()
    while task.wait(0.2) do
        pcall(function()
            ...
            if Core
            and Core:FindFirstChild("HumanoidRootPart")
            and Core:FindFirstChild("Humanoid")
            and Core.Humanoid.Health > 0 then

                EquipWeapon(getgenv().SelectWeapon)

                topos(
                    Core.HumanoidRootPart.CFrame
                    * CFrame.new(0,25,0)
                )
        ...
        if getgenv().AutoEliteHunter and World3 then
            pcall(function()
                ...
                if questGui.Visible then
                    ...
                    if string.find(questText,"Diablo")
                    or string.find(questText,"Urban")
                    or string.find(questText,"Deandre") then

                        for _,v in pairs(replicated:GetChildren()) do

                            if string.find(v.Name,"Diablo")
                            or string.find(v.Name,"Urban")
                            or string.find(v.Name,"Deandre") then
                        ...
                        for _,v in pairs(enemies:GetChildren()) do
                            ...
                            if (
                                string.find(v.Name,"Diablo")
                                or string.find(v.Name,"Urban")
                                or string.find(v.Name,"Deandre")
                            )
                            and Attack.Alive(v) then
                                ...
                                until not getgenv().AutoEliteHunter
...
spawn(function()
    while task.wait(Sec) do
        pcall(function()
            ...
            if not getgenv().AutoAttackRipIndra
            or not World3 then
                return
            end
            ...
            local Boss =
                enemies:FindFirstChild("rip_indra True Form")
                or enemies:FindFirstChild("rip_indra")
            ...
            and Boss.Humanoid.Health > 0 then
                ...
                repeat
                    ...
                    Attack.Kill(
                        Boss,
                        getgenv().AutoAttackRipIndra
                    )

                until not getgenv().AutoAttackRipIndra
                    or not Boss.Parent
                    or Boss.Humanoid.Health <= 0
                ...
                if hrp
                and (hrp.Position - AdminPos.Position).Magnitude > 30 then
        ...
        pcall(function()
            ...
            and World2 then
                ...
                elseif not GetBP("Fist of Darkness") then
                    ...
                    until not getgenv().AutoDarkBladev3
                    or GetBP("Fist of Darkness")
                    or workspace.Enemies:FindFirstChild("Darkbeard")

                    getgenv().AutoFarmChest = false

                elseif GetBP("Fist of Darkness")
                and not workspace.Enemies:FindFirstChild("Darkbeard") then
                ...
                elseif workspace.Enemies:FindFirstChild("Darkbeard") then
                    ...
                    if Darkbeard then
                        ...
                        repeat
                            ...
                            Attack.Kill(
                                Darkbeard,
                                getgenv().AutoDarkBladev3
                            )

                        until not getgenv().AutoDarkBladev3
                        or not Darkbeard.Parent
                        or not Darkbeard:FindFirstChild("Humanoid")
                        or Darkbeard.Humanoid.Health <= 0
                        ...
                        until not getgenv().AutoDarkBladev3
                        or (Root.Position - GravePos.Position).Magnitude < 10

                        fireclickdetector(
                            workspace.Map.GraveIsland.Mountain.Rocks.Button.ClickDetector
                        )
    ...
    while task.wait(0.1) do
        if getgenv().AutoDarkbeard then
            pcall(function()
                ...
                elseif not GetBP("Fist of Darkness")
                and not GetConnectionEnemies("Darkbeard") then
...
spawn(function()
    while task.wait(0.2) do
        pcall(function()
            ...
            if not getgenv().AutoStartRaid then
                return
            end
            ...
            if GetBP("Special Microchip") then
                ...
                if game.PlaceId == 79091703265657 then
                    ...
                    topos(CFrame.new(-6526.8, 305.4, -4800.2))
...
FruitRaid:AddToggle({
Name = "Auto Farm Raid [Kill Aura Only Island 4 - Island 5",
Description = "Kill Aura Only Activates When on Island 4 And on Island 5, because if you kill Aura 5, the island will not receive F",
Default = false,
Callback = function(Value)
...
if not getgenv().AutoDungeon then  
        ...
        local player = game.Players.LocalPlayer  
        local raidUI = player.PlayerGui.Main.TopHUDList.RaidTimer  
...
Volcano:AddToggle({
    ...
    Description = "Prehistoric Island Has Appeared And Boat Has Stopped Auto Off",
    Default = false,
    Callback = function(Value)
...
function CheckBoat()
    for _,v in pairs(workspace.Boats:GetChildren()) do
        if v.Name == getgenv().SelectedBoat
        ...
        if getgenv().FindPrehistoric and World3 then
            pcall(function()
                ...
                if prehistoricIsland then
                    ...
                    Fluent:Notify({
                        Title = "Prehistoric Island",
                        Content = "Island Found! Auto Find Disabled",
                        Duration = 5
                    })
                ...
                if myBoat and myBoat:FindFirstChild("VehicleSeat") then
                    local dist = (myBoat.VehicleSeat.Position - root.Position).Magnitude
                    ...
                    if dist > 2000 then
                        ...
                        pcall(function()
                            game.ReplicatedStorage.Remotes.CommF_:InvokeServer("RemoveBoat")
                        end)
                ...
                if not myBoat then
                    ...
                    if not humanoid.Sit then
                        ...
                        while humanoid.Sit and getgenv().FindPrehistoric do
                            ...
                            local prehistoric = workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island")
                            ...
                            if myBoat.PrimaryPart then
                                speed = myBoat.PrimaryPart.AssemblyLinearVelocity.Magnitude
                            elseif myBoat.VehicleSeat then
                                speed = myBoat.VehicleSeat.AssemblyLinearVelocity.Magnitude
                            end
                            ...
                            if speed <= 1 then
                                ...
                                local prehistoric2 = workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island")

                                if prehistoric2 then
                                    getgenv().FindPrehistoric = false
...
for _,v in pairs(island.Core.VolcanoRocks:GetChildren()) do  
    if v:FindFirstChild("VFXLayer") then  
        local glow = v.VFXLayer:FindFirstChild("At0")  
...
function UseSoulGuitarSkills()
    ...
    local character = game.Players.LocalPlayer.Character
    local gunName = "Soul Guitar"
    
    local tool = (character and character:FindFirstChild(gunName)) or (backpack and backpack:FindFirstChild(gunName))
    
    if tool then
        local mastery = 0
        if tool:GetAttribute("Level") then
            mastery = tool:GetAttribute("Level")
        elseif tool:FindFirstChild("Level") then
            mastery = tool.Level.Value
        else
            pcall(function()
                mastery = game.Players.LocalPlayer.Data.Mastery[gunName].Level.Value
            end)
        end
        ...
        if mastery >= 150 then
            ...
            if character and character:FindFirstChild(gunName) or (backpack and game.Players.LocalPlayer.Character:FindFirstChild(gunName)) then
                Useskills("Gun", "Z")
                task.wait(.4)
                if mastery >= 300 then
                    Useskills("Gun", "X")
...
if not getgenv().AutoDefendVolcano then  
    return  
end  
...
local Golem = GetConnectionEnemies("Lava Golem")  
            ...
            if island  
            and island:FindFirstChild("Core")  
            and island.Core:FindFirstChild("VolcanoRocks") then  

                if not CheckVolcanoLavaDone() then  
                    for _,v in pairs(island.Core.VolcanoRocks:GetChildren()) do  
                        if v:FindFirstChild("VFXLayer")  
                        and v.VFXLayer:FindFirstChild("At0")  
                        and v.VFXLayer.At0:FindFirstChild("Glow") then  

                            local glow = v.VFXLayer.At0.Glow  
...
local function RefreshPlayerList()
    ...
    if PlayerDropdown then
        PlayerDropdown:SetValues(PlrList)
    end
end
        ...
        pcall(function()
            ...
            if not Tool then
                ...
                if getgenv().SelectWeapon then
                    local BackpackTool = Player.Backpack:FindFirstChild(getgenv().SelectWeapon)

                    if BackpackTool and Character:FindFirstChild("Humanoid") then
            ...
            if Tool.ToolTip == "Blox Fruit" and MobAura then
